...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { email, first_name, last_name, country, role, redirect_url, send_invite = true, instructor_type, company_name } = await req.json();

    const isCompanyInstructor = role === "instructor" && instructor_type === "company";

    if (!email || !country || !role) {
      return new Response(
        JSON.stringify({ error: "Email, country, and role are required" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    if (isCompanyInstructor) {
      if (!company_name || !String(company_name).trim()) {
        return new Response(
          JSON.stringify({ error: "Company name is required for company instructors" }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
    ...
    if (role === "instructor") {
      userMeta.instructor_type = instructor_type || "individual";
    }
    if (isCompanyInstructor) {
      userMeta.company_name = String(company_name).trim();
    } else {
    ...
    if (createErr) {
      console.error("admin.createUser failed", { message: createErr.message, role, instructor_type, isCompanyInstructor });
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      const updatePayload: Record<string, any> = {
        ...
        instructor_type: role === "instructor" ? (instructor_type || "individual") : null,
        company_name: isCompanyInstructor ? String(company_name).trim() : null,
        first_name: isCompanyInstructor ? null : first_name,
        last_name: isCompanyInstructor ? null : last_name,
      };
      ...
      if (error) {
        console.error(`Profile update attempt ${attempt + 1} error:`, error.message);
    ...
    if (!profileUpdated) {
      console.warn(`Failed to update profile for user ${userId} after ${MAX_RETRIES} retries.`);
    }
      ...
      try {
        ...
        if (emailSettings) {
          ...
          if (linkErr || !linkData) {
            ...
            const mergeVars = { first_name: first_name || "there", email, link: actionLink || "" };
            ...
            if (sendErr) {
              console.error("Failed to send invite email:", sendErr.message);
            }
          }
        }
      } catch (emailErr: any) {
    ...
    return new Response(
      JSON.stringify({ success: true, userId, email, invite_sent: send_invite }),
