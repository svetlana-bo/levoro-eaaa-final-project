...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const slugBase = trimmedName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "company";
    const slug = `${slugBase}-${Math.random().toString(36).slice(2, 7)}`;

    const { data: company, error: companyErr } = await adminClient
      .from("instructor_companies")
      .insert({ name: trimmedName, slug })
      .select("id, display_id, name, slug")
      .single();
    ...
    for (let idx = 0; idx < instructors.length; idx++) {
      ...
      const memberRole = idx === 0 ? "main_instructor" : "instructor";
      ...
      try {
        ...
        const { error: memberErr } = await adminClient
          ...
          .insert({ user_id: userId, company_id: company.id, member_role: memberRole });
