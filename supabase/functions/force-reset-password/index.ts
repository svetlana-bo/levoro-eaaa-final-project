...
function buildEmailHtml(body: string, heading: string, preheader: string, headerConfig: any, footerConfig: any, senderName: string, senderEmail: string): string {
  ...
  const unsubscribeText = fc.unsubscribeText || "";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isAdmin) {
      const { data: isWebadmin } = await adminClient.rpc("has_role", { _user_id: user.id, _role: "webadmin" });
      if (!isWebadmin) return new Response(JSON.stringify({ error: "Forbidden" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    const { data: { user: targetUser }, error: targetErr } = await adminClient.auth.admin.getUserById(targetUserId);
    if (targetErr || !targetUser) return new Response(JSON.stringify({ error: "User not found" }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    // Generate recovery link (type: "recovery") — does NOT send Supabase's default email
    const { data: linkData, error: linkError } = await adminClient.auth.admin.generateLink({
      type: "recovery",
      email: targetUser.email!,
      options: {
        redirectTo: "https://cozy-dash-portal.lovable.app/reset-password",
      },
    });
    ...
    if (!actionLink) return new Response(JSON.stringify({ error: "No action link generated" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    // Fetch reset template from email_settings
    ...
    if (!emailSettings) {
      return new Response(JSON.stringify({ error: "Email settings not found" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    let bodyHtml = replaceMergeTags(emailSettings.reset_body, mergeVars);

    // If body doesn't contain {{link}}, append a default reset button
    if (!emailSettings.reset_body.toLowerCase().includes("{{link}}")) {
      bodyHtml += `<div style="text-align:center;margin:24px 0;">
        <a href="${actionLink}" style="display:inline-block;padding:12px 32px;background:#1a1a2e;color:#ffffff;text-decoration:none;border-radius:6px;font-weight:600;font-size:15px;">Reset Your Password</a>
      </div>`;
    }

    const heading = replaceMergeTags(emailSettings.reset_heading || "", mergeVars);
    const subject = replaceMergeTags(emailSettings.reset_subject || "Reset Your Password", mergeVars);
    const preheader = replaceMergeTags(emailSettings.reset_preheader || "", mergeVars);
    ...
    const fullHtml = buildEmailHtml(
      ...
      emailSettings.reset_header_config,
      emailSettings.reset_footer_config,
    ...
    if (sendErr) {
      return new Response(JSON.stringify({ error: sendErr.message || JSON.stringify(sendErr) }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    return new Response(JSON.stringify({ success: true, message: `Password reset email sent to ${email}` }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err: any) {
